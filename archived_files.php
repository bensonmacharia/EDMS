...
<td>
<center>
<h2 style="font-family:Verdana, Arial, Helvetica, sans-serif"><b style="color:#0c6500">Archived / Expired Files</b></h2><br/>
</center>
<style>
table {
...
$today = date('Y-m-d');
$result = mysqli_query($connection,"select * FROM documents WHERE evaluate_before < '$today'");
echo "<table class='bordered'>"; // start a table tag in the HTML
echo "<tr>"; echo "<th>Code </th>";echo "<th>Title </th>";echo "<th>Authors </th>";echo "<th>Location </th>"; echo "<th>Date Expired </th>"; echo "</tr>";
while($row = mysqli_fetch_array($result))
 {
 $filename= $row['title'];
 $code= $row['code'];
echo "<tr><td><a href='document_details.php?code=$code'>" . $row['code'] . "</a></td><td>" .$filename . "</td><td>" . $row['authors'] ."</td><td>" . $row['distributed_to'] ."</td><td>". $row['evaluate_before'] . "</td></tr>";
 }
